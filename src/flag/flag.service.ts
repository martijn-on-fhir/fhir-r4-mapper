...
import { Flag, Identifier } from 'fhir-models-r4';
...
export class FlagService implements FhirResourceService<any> {
   ...
   */
  appendIdentifier() {
    const entity = _.find(this.flag.identifier, { system: 'http://www.adapcare.nl/pluriform/ak' });
    ...
    this.flag.addIdentifier(identifier);
  }
  ...
  appendProfile(data: RawEntity) {
    ...
    if (profile) {
      this.flag.meta.addProfile(profile);
    }
  }
