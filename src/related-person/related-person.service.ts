...
import { Identifier, RelatedPerson } from 'fhir-models-r4';
...
export class RelatedPersonService implements FhirResourceService<any> {
   ...
   */
  appendIdentifier() {
    const entity = _.find(this.relatedPerson.identifier, { system: 'http://www.adapcare.nl/pluriform/ak' });

    if (entity) return;

    const identifier = new Identifier({
      system: 'http://www.adapcare.nl/pluriform/ak',
      value: this.relatedPerson.id,
    });

    this.relatedPerson.addIdentifier(identifier);
  }
  ...
  appendProfile(data: RawEntity) {
    ...
    if (profile) {
      this.relatedPerson.meta.addProfile(profile);
    }
  }
