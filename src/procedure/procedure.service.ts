...
import { Identifier, Procedure } from 'fhir-models-r4';
...
export class ProcedureService implements FhirResourceService<any> {
   ...
   */
  appendIdentifier() {
    const entity = _.find(this.procedure.identifier, { system: 'http://www.adapcare.nl/pluriform/ak' });
  ...
  appendProfile(data: RawEntity) {
    ...
    if (profile) {
      this.procedure.meta.addProfile(profile);
    }
  }
